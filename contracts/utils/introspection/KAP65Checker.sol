...
library KAP165Checker {
        ...
        returns (bool)
    {
        ...
        return
            supportsKAP165(account) &&
            _supportsKAP165Interface(account, interfaceId);
    }
    ...
    function getSupportedInterfaces(
        ...
        if (supportsKAP165(account)) {
            ...
            for (uint256 i = 0; i < interfaceIds.length; i++) {
                interfaceIdsSupported[i] = _supportsKAP165Interface(
                    account,
                    interfaceIds[i]
                );
            }
     ...
     */
    function _supportsKAP165Interface(address account, bytes4 interfaceId)
        private
        view
        returns (bool)
    {
        bytes memory encodedParams = abi.encodeWithSelector(
            IKAP165.supportsInterface.selector,
            interfaceId
        );
