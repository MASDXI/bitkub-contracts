    ...
    Pausable
{
    ...
    function adminApprove(address to, uint256 tokenId)
        external
        onlySuperAdmin
        whenNotPaused
    {
...
contract MorningMoonNft is Authorization, KAP721 {
    ...
    function tokenOfOwnerAll(address _owner)
        external
        view
        returns (uint256[] memory)
    {
    ...
    function batchMint(
        ...
        require(
            _to.length == _itemInfo.length &&
                _itemInfo.length == _tokenURI.length,
    ...
    function freeze(address _addr) external {
        if (adminRouter.isSuperAdmin(msg.sender, PROJECT)) {
            ...
            require(
                msg.sender == _addr,
                "Sender must be the same as msg.sender"
            );
            freezed[_addr] = true;
        }
    }
    ...
    function unfreeze(
        ...
        for (uint256 i = 0; i < _burnItemId.length; i++) {
            require(
                ownerOf(_burnItemId[i]) == _addr,
                "The address is not owner"
            );
            burn(_burnItemId[i]);
        }
    ...
    function _beforeTokenTransfer(
        ...
        require(
            from == address(0) || !freezed[from],
            "From address is freezed"
        );
    }
}
